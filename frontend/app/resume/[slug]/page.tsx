...
export async function generateStaticParams() {
  const slugs = getAllSlugs('resume');
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps) {
  const { slug } = await params;
  const item = getContentBySlug('resume', slug);
  if (!item) return {};

  return generateSeoFromFrontmatter(item.frontmatter, 'resume');
}

export default async function ResumeGuidePage({ params }: PageProps) {
  const { slug } = await params;
  const item = getContentBySlug('resume', slug);
  ...
  const related = getRelatedContent(slug, 'resume', 6);

  const schemas = [
    generateOrganizationSchema(),
    generateWebsiteSchema(),
    generateArticleSchema(item.frontmatter, 'resume'),
    ...
    <>
      ...
      <div className="min-h-screen bg-paper text-ink flex flex-col justify-between">
        ...
        <main className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
          <Breadcrumbs
            items={[
              { name: 'Resume Guides', url: '/resume' },
              { name: item.frontmatter.title, url: `/resume/${slug}` },
            ]}
          />

          <ContentHero frontmatter={item.frontmatter} collection="resume" />
          ...
          <RelatedContent relatedItems={related} title="Popular Resume Guides for Tech Roles" />
        </main>
        <Footer />
      </div>
    </>
