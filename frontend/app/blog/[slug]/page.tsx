...
export async function generateMetadata({ params }: PageProps) {
  ...
  return generateSeoFromFrontmatter(item.frontmatter, 'blog');
}
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  const related = getRelatedContent(slug, 'blog', 6);

  const schemas = [
    generateOrganizationSchema(),
    generateWebsiteSchema(),
    generateArticleSchema(item.frontmatter, 'blog'),
      ...
      <div className="min-h-screen bg-paper text-ink flex flex-col justify-between">
        ...
        <main className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
          <Breadcrumbs
            items={[
              { name: 'Blog', url: '/blog' },
              { name: item.frontmatter.title, url: `/blog/${slug}` },
            ]}
          />

          <ContentHero frontmatter={item.frontmatter} collection="blog" />
          ...
          <RelatedContent relatedItems={related} title="Latest Resume & Career Insights" />
        </main>
        <Footer />
      </div>
    </>
